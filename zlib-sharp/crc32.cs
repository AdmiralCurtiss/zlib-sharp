...
public static ulong crc32_z(
    ulong crc,
    byte[] buf_array,
    long buf_index,
    ulong len)
{
    if (buf_array == null) return 0UL;

    // TODO faster crc calc
    //if (true) {
    //    if (System.BitConverter.IsLittleEndian)
    //        return crc32_little(crc, buf_array, len);
    //    else
    //        return crc32_big(crc, buf_array, len);
    //}
    crc = crc ^ 0xffffffffUL;
    while (len >= 8) {
        crc = crc_table[((int)crc ^ (buf_array[buf_index++])) & 0xff] ^ (crc >> 8);
        crc = crc_table[((int)crc ^ (buf_array[buf_index++])) & 0xff] ^ (crc >> 8);
        crc = crc_table[((int)crc ^ (buf_array[buf_index++])) & 0xff] ^ (crc >> 8);
        crc = crc_table[((int)crc ^ (buf_array[buf_index++])) & 0xff] ^ (crc >> 8);
        crc = crc_table[((int)crc ^ (buf_array[buf_index++])) & 0xff] ^ (crc >> 8);
    ...
    return crc ^ 0xffffffffUL;
		...
		public static ulong crc32_(
			ulong crc,
			byte[] buf_array,
			long buf_index,
			uint len) {
			return crc32_z(crc, buf_array, buf_index, len);
		}
